  ...
  <el-form :model="SalesDeliveryDetail" :label-width=LabeWidth label-position="right" v-loading="loading">
   ...
   <el-form :inline="true" :model="formInline" class="demo-form-inline">
    ...
    <el-form-item class="purchaseearehouse1" label="出库经办:" :label-width=LabeWidth>
     <el-input v-model="SalesDeliveryDetail.handlerId" placeholder="出库经办"></el-input>
    </el-form-item>
   </el-form>

   <el-form :inline="true" :model="formInline" class="demo-form-inline">
     <el-form-item class="purchaseearehouse1" label="源单类型:" :label-width=LabeWidth required>
     <el-select v-model="SalesDeliveryDetail.sourceType" placeholder="源单类型" class="option2">   
...
export default defineComponent({
  ...
  setup (props, context) {
    ...
    const addSubTable = () => {
      SalesDeliverySubTableDetail.value.map((p:any) => {
        if (!p.ioBillDetailId) {
          const data:any = p
          data.modifiedDate = undefined
          data.supplierId = SalesDeliveryDetail.value.supplierId
