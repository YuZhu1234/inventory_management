<template>
  ...
  >
    ...
    :loadSalesDeliverylist="loadSalesDeliverylist" 
...
import SalesDeliveryDetail from './SalesDeliveryDetail.vue'
...
export default defineComponent({
  ...
  setup () {
    ...
    const handleCurrentChange = (val: number) :void => {
      loadSalesDeliverylist(val)
    }

    const handleReset = () => {
      dateValue.value = []
      searchId.value = ''
      loadSalesDeliverylist(1)
    }

    const handleSearch = () => {
      loadSalesDeliverylist(1, dateValue.value[1], dateValue.value[0], searchId.value)
    }
    ...
    const loadSalesDeliverylist = (current_page:number, updateEndTime?:string, updateStartTime?:string, searchId?:string) :void => {
      AxiosApi.get(`billHeader/list?pageNum=${current_page}&stockIoName=销售出库${updateEndTime ? `&updateEndTime=${updateEndTime}&updateStartTime=${updateStartTime}` : '&spageSize=10'}${searchId ? `&billNo=${searchId}` : ''}`)
        .then((res:AxiosResponse) => {
          SalesDeliverylist.value = res.data.result
