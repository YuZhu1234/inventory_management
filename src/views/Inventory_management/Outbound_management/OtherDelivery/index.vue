<template>
  ...
  >
    ...
    :loadOtherDeliverylist="loadOtherDeliverylist" 
...
import OtherDeliveryDetail from './OtherDeliveryDetail.vue'
...
export default defineComponent({
  ...
  setup () {
    ...
    const handleCurrentChange = (val: number) :void => {
      loadOtherDeliverylist(val)
    }

    const handleReset = () => {
      dateValue.value = []
      searchId.value = ''
      loadOtherDeliverylist(1)
    }

    const handleSearch = () => {
      loadOtherDeliverylist(1, dateValue.value[1], dateValue.value[0], searchId.value)
    }
    ...
    const loadOtherDeliverylist = (current_page:number, updateEndTime?:string, updateStartTime?:string, searchId?:string) :void => {
      AxiosApi.get(`billHeader/list?pageNum=${current_page}&stockIoName=其他出库${updateEndTime ? `&updateEndTime=${updateEndTime}&updateStartTime=${updateStartTime}` : '&spageSize=10'}${searchId ? `&billNo=${searchId}` : ''}`)
        .then((res:AxiosResponse) => {
          OtherDeliverylist.value = res.data.result
