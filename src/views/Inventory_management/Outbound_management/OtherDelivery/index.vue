...
export default defineComponent({
  ...
  setup () {
    ...
    const loadOtherDeliverylist = (current_page:number, updateEndTime?:string, updateStartTime?:string, searchId?:string) :void => {
      loading.value = true
      AxiosApi.get(`billHeader/list?pageNum=${current_page}&stockIoName=其他出库${updateEndTime ? `&updateEndTime=${updateEndTime}&updateStartTime=${updateStartTime}` : '&pageSize=10'}${searchId ? `&billNo=${searchId}` : ''}`)
