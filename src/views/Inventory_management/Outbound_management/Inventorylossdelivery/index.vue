...
import InventorylossdeliveryDetail from './InventorylossdeliveryDetail.vue'
...
export default defineComponent({
  ...
  setup () {
    ...
    const loadInventorylossdeliverylist = (current_page:number, updateEndTime?:string, updateStartTime?:string, searchId?:string) :void => {
      AxiosApi.get(`billHeader/list?pageNum=${current_page}&stockIoName=盘亏出库${updateEndTime ? `&updateEndTime=${updateEndTime}&updateStartTime=${updateStartTime}` : '&spageSize=10'}${searchId ? `&billNo=${searchId}` : ''}`)
