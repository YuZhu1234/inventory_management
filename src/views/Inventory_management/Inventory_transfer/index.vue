<template>
  ...
  >
    ...
    :loadInventoryTransferlist="loadInventoryTransferlist" 
...
import InventoryTransferDetail from './InventoryTransferDetail.vue'
...
export default defineComponent({
  ...
  setup () {
    ...
    const InventoryTransferlist = ref([{}])
    const total = ref(0)
    ...
    const handleCurrentChange = (val: number) :void => {
      ...
      loadInventoryTransferlist(val)
    }

    const handleReset = () => {
      dateValue.value = []
      searchId.value = ''
      loadInventoryTransferlist(1)
    }

    const handleSearch = () => {
      loadInventoryTransferlist(1, dateValue.value[1], dateValue.value[0], searchId.value)
    }
    ...
    const loadInventoryTransferlist = (current_page:number, updateEndTime?:string, updateStartTime?:string, searchId?:string) :void => {
      loading.value = true
      AxiosApi.get(`billHeader/list?pageNum=${current_page}&stockIoName=仓库调拨${updateEndTime ? `&updateEndTime=${updateEndTime}&updateStartTime=${updateStartTime}` : '&spageSize=10'}${searchId ? `&billNo=${searchId}` : ''}`)
        .then((res:AxiosResponse) => {
          InventoryTransferlist.value = res.data.result
