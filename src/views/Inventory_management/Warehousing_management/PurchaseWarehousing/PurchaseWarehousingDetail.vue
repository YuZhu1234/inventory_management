<template>
  ...
  <el-form :model="PurchaseWarehousingDetail" :label-width=LabeWidth label-position="right" v-loading="loading">
   ...
   <el-form :inline="true" :model="formInline" class="demo-form-inline">
     ...
     <el-select v-model="PurchaseWarehousingDetail.sourceType" placeholder="源单类型" class="option2">   
...
export default defineComponent({
  ...
  setup (props, context) {
    ...
    const addSubTable = () => {
      PurchaseWarehousingSubTableDetail.value.map((p:any) => {
        if (!p.ioBillDetailId) {
          const data:any = p
          data.modifiedDate = undefined
          data.supplierId = PurchaseWarehousingDetail.value.supplierId
