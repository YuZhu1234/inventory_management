<template>
  <Card>
   <el-form :inline="true" class="demo-form">
    ...
    <el-table 
      :data="SaturationOfStackRoomlist" 
  ...
  >
    ...
    :loadSaturationOfStackRoomlist="loadSaturationOfStackRoomlist" 
...
import SaturationOfStackRoomDetail from './SaturationOfStackRoomDetail.vue'
import { AxiosApi } from '../../../../utils/api'
import { AxiosResponse } from 'axios'
import { ElMessage } from 'element-plus'

export default defineComponent({
  name:'SaturationOfStackRoom',
  components:{
    Refresh,
    Search,
    Plus,
  ...
  setup () {
    ...
    const SaturationOfStackRoomlist = ref([{}])
    const total = ref(0)
    ...
    const handleCurrentChange = (val: number) :void => {
      loadSaturationOfStackRoomlist(val)
    }

    const handleReset = () => {
      dateValue.value = []
      searchId.value = ''
      loadSaturationOfStackRoomlist(1)
    }

    const handleSearch = () => {
      loadSaturationOfStackRoomlist(1, dateValue.value[1], dateValue.value[0], searchId.value)
    }
    ...
    const loadSaturationOfStackRoomlist = (current_page:number, updateEndTime?:string, updateStartTime?:string, searchId?:string) :void => {
      AxiosApi.get(`billHeader/list?pageNum=${current_page}&stockIoName=涨库入库${updateEndTime ? `&updateEndTime=${updateEndTime}&updateStartTime=${updateStartTime}` : '&spageSize=10'}${searchId ? `&billNo=${searchId}` : ''}`)
        .then((res:AxiosResponse) => {
          SaturationOfStackRoomlist.value = res.data.result
