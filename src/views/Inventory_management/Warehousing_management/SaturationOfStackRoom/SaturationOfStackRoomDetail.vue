...
export default defineComponent({
  ...
  setup (props, context) {
    ...
    const handleSave = (type:string) => {
      ...
      if (props.edit_type === 'edit') {
        if (type === 'header') {
          ...
          AxiosApi.post('billDetail/add', JSON.stringify(SaturationOfStackRoomSubTableDetail.value[SaturationOfStackRoomSubTableDetail.value.length - 1]))
            .then((res) => {
              console.log(res)
              success('保存成功')
              props.loadSaturationOfStackRoomlist(1)
        ...
        if (type === 'header') {
          ...
          AxiosApi.post('billDetail/add', JSON.stringify(SaturationOfStackRoomSubTableDetail.value))
            .then((res) => {
              console.log(res)
              success('保存成功')
              props.loadSaturationOfStackRoomlist(1)
    ...
    const loadSaturationOfStackRoomHeaderDetail = () => {
      loading.value = true
      AxiosApi.get(`billHeader/find?billNo=${props.billNo}&stockIoName=涨库入库`)
        .then((res:AxiosResponse) => {
