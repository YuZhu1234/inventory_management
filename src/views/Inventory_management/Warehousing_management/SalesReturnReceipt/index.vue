<template>
  ...
  >
    ...
    :loadSalesReturnReceiptlist="loadSalesReturnReceiptlist" 
...
import SalesReturnReceiptDetail from './SalesReturnReceipt.vue'
...
export default defineComponent({
  ...
  setup () {
    ...
    const SalesReturnReceiptlist = ref([{}])
    const total = ref(0)
    ...
    const handleCurrentChange = (val: number) :void => {
      loadSalesReturnReceiptlist(val)
    }

    const handleReset = () => {
      dateValue.value = []
      searchId.value = ''
      loadSalesReturnReceiptlist(1)
    }

    const handleSearch = () => {
      loadSalesReturnReceiptlist(1, dateValue.value[1], dateValue.value[0], searchId.value)
    }
    ...
    const loadSalesReturnReceiptlist = (current_page:number, updateEndTime?:string, updateStartTime?:string, searchId?:string) :void => {
      AxiosApi.get(`billHeader/list?pageNum=${current_page}&stockIoName=销售入库-红字${updateEndTime ? `&updateEndTime=${updateEndTime}&updateStartTime=${updateStartTime}` : '&spageSize=10'}${searchId ? `&billNo=${searchId}` : ''}`)
        .then((res:AxiosResponse) => {
          SalesReturnReceiptlist.value = res.data.result
