<template>
  <Card>
   <el-form :inline="true" class="demo-form">
    ...
    <el-table 
      :data="InventoryGainWarehousinglist" 
  ...
  >
    ...
    :loadInventoryGainWarehousinglist="loadInventoryGainWarehousinglist" 
...
import InventoryGainWarehousingDetail from './InventoryGainWarehousinglist.vue'
import { AxiosApi } from '../../../../utils/api'
import { AxiosResponse } from 'axios'
import { ElMessage } from 'element-plus'

export default defineComponent({
  name:'InventoryGainWarehousing',
  components:{
    Refresh,
    Search,
    Plus,
  ...
  setup () {
    ...
    const InventoryGainWarehousinglist = ref([{}])
    const total = ref(0)
    ...
    const handleCurrentChange = (val: number) :void => {
      loadInventoryGainWarehousinglist(val)
    }

    const handleReset = () => {
      dateValue.value = []
      searchId.value = ''
      loadInventoryGainWarehousinglist(1)
    }

    const handleSearch = () => {
      loadInventoryGainWarehousinglist(1, dateValue.value[1], dateValue.value[0], searchId.value)
    }
    ...
    const loadInventoryGainWarehousinglist = (current_page:number, updateEndTime?:string, updateStartTime?:string, searchId?:string) :void => {
      AxiosApi.get(`billHeader/list?pageNum=${current_page}&stockIoName=盘盈入库${updateEndTime ? `&updateEndTime=${updateEndTime}&updateStartTime=${updateStartTime}` : '&spageSize=10'}${searchId ? `&billNo=${searchId}` : ''}`)
        .then((res:AxiosResponse) => {
          InventoryGainWarehousinglist.value = res.data.result
