<template>
  ...
  >
    ...
    :loadInventoryGainlist="loadInventoryGainlist" 
...
import InventoryGainDetail from './InventoryGainWarehousinglist.vue'
...
export default defineComponent({
  ...
  setup () {
    ...
    const handleCurrentChange = (val: number) :void => {
      ...
      loadInventoryGainlist(val)
    }

    const handleReset = () => {
      dateValue.value = []
      searchId.value = ''
      loadInventoryGainlist(1)
    }

    const handleSearch = () => {
      loadInventoryGainlist(1, dateValue.value[1], dateValue.value[0], searchId.value)
    }
    ...
    const loadInventoryGainlist = (current_page:number, updateEndTime?:string, updateStartTime?:string, searchId?:string) :void => {
      loading.value = true
      AxiosApi.get(`billHeader/list?pageNum=${current_page}&stockIoName=盘盈入库${updateEndTime ? `&updateEndTime=${updateEndTime}&updateStartTime=${updateStartTime}` : '&pageSize=10'}${searchId ? `&billNo=${searchId}` : ''}`)
        .then((res:AxiosResponse) => {
          InventoryGainlist.value = res.data.result
