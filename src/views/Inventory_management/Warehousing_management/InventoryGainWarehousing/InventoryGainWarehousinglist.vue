...
export default defineComponent({
  name:'InventoryGainWarehousinglist',
  components:{
    Plus,
    Minus
  },
    ...
    const handleSave = (type:string) => {
      ...
      if (props.edit_type === 'edit') {
        if (type === 'header') {
          ...
          AxiosApi.post('billDetail/add', JSON.stringify(InventoryGainWarehousingSubTableDetail.value[InventoryGainWarehousingSubTableDetail.value.length - 1]))
            .then((res) => {
              console.log(res)
              success('保存成功')
              props.loadInventoryGainWarehousinglist(1)
        ...
        if (type === 'header') {
          ...
          AxiosApi.post('billDetail/add', JSON.stringify(InventoryGainWarehousingSubTableDetail.value))
            .then((res) => {
              console.log(res)
              success('保存成功')
              props.loadInventoryGainWarehousinglist(1)
